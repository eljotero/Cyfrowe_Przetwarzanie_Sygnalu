...
class CompareGUI(QMainWindow):
    def __init__(self, title, values, parent=None):
        ...
        if values is not None:
            ...
            self.enob_line_edit = QLabel(self)
            form_layout = QVBoxLayout()
            layout1 = QHBoxLayout()
            layout1.addWidget(QLabel("Błąd średniokwadratowy:"))
            layout1.addWidget(self.mse_line_edit)
            form_layout.addLayout(layout1)
            layout2 = QHBoxLayout()
            layout2.addWidget(QLabel("Stosunek sygnał - szum:"))
            layout2.addWidget(self.snr_line_edit)
            form_layout.addLayout(layout2)
            layout3 = QHBoxLayout()
            layout3.addWidget(QLabel("Szczytowy stosunek sygnał - szum:"))
            layout3.addWidget(self.psnr_line_edit)
            form_layout.addLayout(layout3)
            layout4 = QHBoxLayout()
            layout4.addWidget(QLabel("Maksymalna różnica:"))
            ...
            self.enob_line_edit.setText(str(values[4]))
            ...
            self.enob_line_edit.setStyleSheet("border: 1px solid black")
